...
----------------------------------------------------
Fires once via a separate cron entry (~16:10 ET, after tick.py's own
flatten deadline has had time to settle). Ported from the sibling ORB
project's eod_report.py with targeted edits: candidates -> trend_setups,
and the reflection prompt retitled for this strategy. Sequence,
reconcile/save/file-write logic, and the LLM-touchpoint scope boundary
are all unchanged.

This is the ONLY place an LLM touches this project. The entry/exit/
sizing path (ema_trend/signal.py + tools/trading/rules_engine.py) is
...
def build_trade_table(positions: list, setups_by_id: dict) -> str:
    ...
    for p in positions:
        ...
        rationale_parts = []
        if setup.get("ema_used"):
            rationale_parts.append(f"pullback to {setup['ema_used']}")
        ...
        rationale_parts.append(f"{p['direction']} resumption break")
...
def build_context(trading_date: str, positions: list, setups: list, account_state: dict, daily_state: dict, breaches: list) -> str:
    ...
    parts = [
        ...
        "\nTREND-SETUP OUTCOMES TODAY:",
        f"  {status_counts if status_counts else 'No trend setups found (no ticker qualified as a clean trend today)'}",
        f"\nRULE BREACHES TODAY:\n{breach_lines}",
    ]
    return "\n".join(parts)
...
9/20 EMA trend-pullback-and-resumption day-trading strategy operated
under a prop-firm compliance rulebook. Given today's actual trade log
and compliance state below, write a short reflection with exactly these
three headers:
...
(e.g. "MDB's pullback to the 9 EMA at 10:15 resumed cleanly and hit
target" is useful; "risk management is important" is not). Do not
restate the rulebook's rules in general terms. If there were zero
trades today, say so plainly and comment on whether that reflects a
genuinely choppy/non-trending day across the universe versus the
clean-trend filter being too strict — but do not speculate beyond what
the data shows. If many setups were abandoned via trend failure or
trend flip, comment on whether that's a sign the clean-trend filter's
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="EMA Trend Pullback EOD Report")
