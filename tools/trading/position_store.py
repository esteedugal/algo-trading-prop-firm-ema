...
Position Store
----------------
SQLite-backed state for the 9/20 EMA trend-pullback strategy. Single-
writer, single-process (1-minute cron ticks), stdlib sqlite3 — same
discipline as the sibling ORB project's position store.

Seven tables. Five are byte-for-byte identical in shape and CRUD
behavior to the ORB sibling's position_store.py (account_state,
daily_state minus two ORB-named columns, positions, breaches,
eod_reports) — none of them ever referenced ORB-specific concepts.

The two that differ replace ORB's single write-once `candidates` table:
  trend_state  - one row per (trading_date, ticker), CONTINUOUSLY
                 MUTATED as new 5-min bars arrive (not write-once like
                 ORB's opening-range snapshot) — tracks which phase a
                 ticker is in: no_trend -> clean_trend -> watching_resumption
                 -> position_open -> back to clean_trend/no_trend.
  trend_setups - one row per DISTINCT pullback->resumption cycle. A
                 ticker can generate multiple trend_setups rows in one
                 day (trend, pullback, resumption, trade closes, a NEW
                 pullback cycle forms later) — deliberately no
                 UNIQUE(trading_date, ticker) constraint, unlike ORB's
                 candidates table. The positions table's own partial
                 unique index (below) already prevents two CONCURRENT
                 open positions on the same ticker; it does not (and
                 should not) prevent sequential same-day re-entry.
"""
...
CREATE TABLE IF NOT EXISTS trend_state (
    ...
    phase                         TEXT    NOT NULL DEFAULT 'no_trend',
        -- no_trend -> clean_trend -> watching_resumption -> position_open -> (clean_trend/no_trend)
    direction                      TEXT,
    ema9                            REAL,
    ema20                            REAL,
    last_bar_ts                       TEXT,   -- idempotency guard against reprocessing the same 5-min bar
    trend_started_at                   TEXT,  -- bar ts of the last genuine EMA cross -- bounds swing lookback
    clean_since_bar_ts                  TEXT,
    pullback_bar_high                    REAL,
    pullback_bar_low                      REAL,
    pullback_bar_ts                        TEXT,
    active_setup_id                         INTEGER REFERENCES trend_setups(id),
...
CREATE TABLE IF NOT EXISTS trend_setups (
    id                         INTEGER PRIMARY KEY AUTOINCREMENT,
    trading_date                TEXT    NOT NULL,
    ticker                       TEXT    NOT NULL,
    cycle_number                  INTEGER NOT NULL,
    direction                      TEXT    NOT NULL,
    ema_used                        TEXT,    -- 'ema9' | 'ema20' -- which EMA the pullback touched
    pullback_bar_high                REAL,
    pullback_bar_low                  REAL,
    pullback_bar_ts                    TEXT,
    swing_ref_price                     REAL,  -- swing point used for the fixed target
    swing_ref_ts                         TEXT,
    entry_trigger                         REAL,
    stop_price                             REAL,
    target_price                            REAL,
    risk_per_share                           REAL,
    stop_basis                                TEXT,  -- 'ema' | 'swing' -- whichever was tighter
    status                                     TEXT    NOT NULL DEFAULT 'watching',
        -- watching -> triggered / invalidated_halt_proxy / skipped_* / expired_eod
        --           / abandoned_trend_flip / abandoned_trend_failure
    created_at                                  TEXT    NOT NULL,
    updated_at                                   TEXT    NOT NULL
...
CREATE TABLE IF NOT EXISTS positions (
    id                         INTEGER PRIMARY KEY AUTOINCREMENT,
    candidate_id                 INTEGER NOT NULL REFERENCES trend_setups(id),
    trading_date                  TEXT    NOT NULL,
    ticker                          TEXT    NOT NULL,
    direction                        TEXT    NOT NULL,
...
def get_trend_state(trading_date: str, ticker: str, db_path: str = DB_PATH) -> Optional[dict]:
    conn = _connect(db_path)
    try:
        row = conn.execute(
            "SELECT * FROM trend_state WHERE trading_date = ? AND ticker = ?",
            (trading_date, ticker),
        ).fetchone()
        return _row_to_dict(row) if row else None
    finally:
...
def create_trend_state(trading_date: str, ticker: str, db_path: str = DB_PATH) -> dict:
    conn = _connect(db_path)
    now = datetime.now().isoformat()
    try:
        conn.execute(
            "INSERT INTO trend_state (trading_date, ticker, created_at, updated_at) VALUES (?, ?, ?, ?)",
            (trading_date, ticker, now, now),
        )
        conn.commit()
    finally:
        conn.close()
    return get_trend_state(trading_date, ticker, db_path)


def get_or_create_trend_state(trading_date: str, ticker: str, db_path: str = DB_PATH) -> dict:
    existing = get_trend_state(trading_date, ticker, db_path)
    return existing if existing else create_trend_state(trading_date, ticker, db_path)


def update_trend_state(trading_date: str, ticker: str, db_path: str = DB_PATH, **fields) -> None:
    if not fields:
        return
    conn = _connect(db_path)
    try:
        ...
        conn.execute(
            f"UPDATE trend_state SET {set_clause}, updated_at=? WHERE trading_date=? AND ticker=?", values
        )
        conn.commit()
    finally:
        conn.close()
...
def list_trend_states_for_date(trading_date: str, db_path: str = DB_PATH) -> list:
    conn = _connect(db_path)
    try:
        rows = conn.execute(
            "SELECT * FROM trend_state WHERE trading_date = ?", (trading_date,)
        ).fetchall()
...
def create_setup(
    ...
    try:
        cur = conn.execute(
            """
            INSERT INTO trend_setups (trading_date, ticker, cycle_number, direction, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?, ?)
            """,
...
def get_setup(setup_id: int, db_path: str = DB_PATH) -> Optional[dict]:
    conn = _connect(db_path)
    try:
        row = conn.execute("SELECT * FROM trend_setups WHERE id = ?", (setup_id,)).fetchone()
        return _row_to_dict(row) if row else None
    finally:
        conn.close()
...
def update_setup(setup_id: int, db_path: str = DB_PATH, **fields) -> None:
    ...
    try:
        ...
        conn.execute(f"UPDATE trend_setups SET {set_clause}, updated_at=? WHERE id=?", values)
        conn.commit()
    finally:
        conn.close()
...
def list_setups_for_date(trading_date: str, db_path: str = DB_PATH) -> list:
    ...
    try:
        rows = conn.execute(
            "SELECT * FROM trend_setups WHERE trading_date = ? ORDER BY created_at",
            (trading_date,),
        ).fetchall()
        return [_row_to_dict(r) for r in rows]
    finally:
...
def list_watching_setups(trading_date: str, db_path: str = DB_PATH) -> list:
    ...
    try:
        rows = conn.execute(
            "SELECT * FROM trend_setups WHERE trading_date = ? AND status = 'watching' ORDER BY created_at",
            (trading_date,),
        ).fetchall()
        return [_row_to_dict(r) for r in rows]
    finally:
...
def get_open_position_db(ticker: str, db_path: str = DB_PATH) -> Optional[dict]:
    """DB-recorded pending/open position for this ticker (name mirrors the
    ORB sibling's get_open_position_db, suffixed _db to avoid confusion
    with Alpaca's own get_open_position() in order_manager)."""
    conn = _connect(db_path)
    try:
        row = conn.execute(
...
def create_pending_position(
    ...
    this ticker, or client_order_id was already used — both are
    meaningful "already attempted" signals the caller should treat as a
    skip, not swallow silently. candidate_id here is a trend_setups.id.
    """
    conn = _connect(db_path)
    now = datetime.now().isoformat()
    try:
...
def correct_position(position_id: int, qty: float, fill_price: float, db_path: str = DB_PATH) -> None:
    ...
    position ledger — for use when an audit finds drift (e.g. a partial
    fill that kept filling in the background after an early snapshot)."""
    conn = _connect(db_path)
    try:
        conn.execute(
...
def mark_position_failed(
    ...
    fill MUST cancel the live order at Alpaca before calling this, not
    just abandon it."""
    conn = _connect(db_path)
    try:
        conn.execute(
